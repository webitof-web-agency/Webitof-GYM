...
const FaqItem = ({ item, i18n, isOpen, onToggle }) => (
    <div
        className={`rounded-2xl border transition-all duration-200 overflow-hidden ${isOpen ? 'border-[#5572fc]/30 shadow-[0_4px_20px_-6px_rgba(85,114,252,0.12)]' : 'border-slate-100 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)]'} bg-white`}
    >
        <button
            onClick={onToggle}
        ...
        >
            <div className='flex items-start gap-3 min-w-0'>
                <div className={`shrink-0 flex h-7 w-7 items-center justify-center rounded-lg transition-colors ${isOpen ? 'bg-[#5572fc] text-white' : 'bg-slate-100 text-gray-400'}`}>
                    <FiHelpCircle size={14} />
                </div>
                <span className={`text-[14px] font-bold leading-snug transition-colors ${isOpen ? 'text-[#5572fc]' : 'text-gray-800'}`}>
                    {item?.question[i18n.langCode]}
                </span>
            </div>
            <FiChevronDown
                size={18}
                className={`shrink-0 text-gray-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#5572fc]' : ''}`}
            />
        </button>
            ...
            <div className='container py-16 lg:py-24'>
                ...
                <div className='max-w-2xl mb-14'>
                    <div className='inline-flex items-center gap-2 rounded-full border border-[#5572fc]/25 bg-[#5572fc]/8 px-4 py-1.5 mb-5'>
                        ...
                        <span className='text-[11px] font-black text-[#5572fc] uppercase tracking-widest'>{i18n?.t('Help Center')}</span>
                    </div>
                    <h1 className='text-3xl lg:text-4xl font-extrabold text-gray-800 tracking-tight leading-tight mb-4'>
                        {i18n?.t('Frequently Asked')} <span className='text-[#5572fc]'>{i18n?.t('Questions')}</span>
                    </h1>
                ...
                {/* CTA strip */}
                <div className='mt-14 max-w-3xl rounded-2xl bg-gradient-to-r from-[#3a52c4] to-[#5572fc] p-8 flex flex-col sm:flex-row items-center justify-between gap-4'>
                    <div>
                    ...
                    <a
                        href='/contact'
                        className='shrink-0 inline-flex items-center gap-2 rounded-xl bg-white px-6 py-2.5 text-[13px] font-bold text-[#5572fc] shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5'
                    >
